...
export function NotesApp({ initialNotes, userName }: { initialNotes: Note[]; userName: string }) {
  ...
  return (
    <main className="flex h-dvh min-h-[560px] overflow-hidden bg-background text-foreground">
      {screen === 'files' && <aside className={cn('absolute inset-y-0 left-0 z-30 flex w-[292px] shrink-0 -translate-x-full flex-col border-r bg-card transition-transform md:relative md:translate-x-0', mobileSidebar && 'translate-x-0')}>
